...
Describe 'Assessment.ActiveDirectory finding contract v1' {

    BeforeAll {
                $ModuleRoot = (Resolve-Path "$PSScriptRoot/../..").Path
        $ModuleManifest = Join-Path $ModuleRoot 'Assessment.ActiveDirectory.psd1'
        $HelperPath = Join-Path $PSScriptRoot 'AssessmentADContractTestHelpers.ps1'
        ...
        if (-not (Test-Path -LiteralPath $HelperPath -PathType Leaf)) {
            throw "AssessmentADContractTestHelpers.ps1 not found: $HelperPath"
        }
        ...
        if (-not (Get-Command New-TestAssessmentADFinding -ErrorAction SilentlyContinue)) {
            throw 'Test helper New-TestAssessmentADFinding was not loaded.'
        }
    }
    ...
    It 'creates a finding with the required contract properties' {
        ...
        $Finding = New-TestAssessmentADFinding

        Assert-AssessmentADFindingContract `
            -Result $Finding `
