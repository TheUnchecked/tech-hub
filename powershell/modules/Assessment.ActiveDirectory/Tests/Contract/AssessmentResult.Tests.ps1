...
Describe 'AssessmentADAssessmentResult' {
BeforeAll {
    ...
    Import-Module -Name (Join-Path -Path $ModuleRoot -ChildPath 'Assessment.ActiveDirectory.psm1') -Force -ErrorAction Stop
}
    AfterAll {
        Remove-Module -Name Assessment.ActiveDirectory -Force -ErrorAction SilentlyContinue
    }
