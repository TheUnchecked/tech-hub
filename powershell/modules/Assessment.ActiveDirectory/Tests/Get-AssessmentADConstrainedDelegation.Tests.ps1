...
Describe 'Get-AssessmentADConstrainedDelegation provider migration' {
    ...
    $ModulePath = Join-Path -Path $ModuleRoot -ChildPath 'Assessment.ActiveDirectory.psm1'
    ...
    AfterAll { Remove-Module -Name Assessment.ActiveDirectory -Force -ErrorAction SilentlyContinue }
    ...
    It 'creates a provider when invoked without one' {
        Mock New-AssessmentADProvider -ModuleName Assessment.ActiveDirectory -MockWith { New-TestConstrainedProvider -Objects @($Script:User) }
        @(Get-AssessmentADConstrainedDelegation -Server 'dc01.example.test').Count | Should -Be 1
        Should -Invoke New-AssessmentADProvider -ModuleName Assessment.ActiveDirectory -ParameterFilter { $Server -eq 'dc01.example.test' } -Times 1
    }
