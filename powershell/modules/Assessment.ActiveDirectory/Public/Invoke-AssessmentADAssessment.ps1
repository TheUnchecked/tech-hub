...
function Invoke-AssessmentADAssessment {
    ...
    $Assessment.Metadata = [PSCustomObject][ordered]@{
        Engine        = 'Assessment.ActiveDirectory'
        EngineVersion = '1.0.0'
        ...
        @(
            $Definitions |
                Where-Object {
                    ...
                    $RequiredProviders -contains 'AssessmentAD' -or
                    $RequiredProviders -contains 'AssessmentADProvider'
                }
        ).Count -gt 0
    ...
    if (
        ...
        try {
            ...
            Write-Verbose `
                'Creating one AssessmentADProvider for this assessment.'
            ...
            Write-Verbose `
                (
                    'Unable to create AssessmentADProvider: {0}' -f
                    $_.Exception.Message
                )
        }
    }
