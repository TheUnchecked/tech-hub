...
Before execution, if at least one selected definition requires `TechHubAD` or `TechHubADProvider`, the engine creates one `TechHubADProvider` with `-Server` unless the caller supplied `-Provider`. That same instance is reused for every provider-aware check in the assessment. Legacy checks that do not expose `-Provider` keep their existing invocation behavior.
