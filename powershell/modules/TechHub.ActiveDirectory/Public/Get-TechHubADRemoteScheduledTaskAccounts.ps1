...
function Get-TechHubADRemoteScheduledTaskAccounts {
    [CmdletBinding()]
    param(
        [Parameter(
            Mandatory,
    ...
    begin {
        ...
        function Convert-TechHubTaskIdentity {
            param(
                [Parameter(Mandatory)]
                [string]$Identity
            )
        ...
        function Get-TechHubTasksViaCim {
            param(
                [Parameter(Mandatory)]
                [string]$Target
            )
            ...
            try {
                ...
                foreach ($task in @($tasks)) {
                    ...
                    if ($rawAccount) {
                        $account = Convert-TechHubTaskIdentity `
                            -Identity $rawAccount
                    }
        ...
        function Get-TechHubTasksViaCom {
            param(
                [Parameter(Mandatory)]
                [string]$Target
            )
    ...
    process {
        ...
        try {
            Get-TechHubTasksViaCim -Target $ComputerName
        }
        catch {
            ...
            try {
                Get-TechHubTasksViaCom -Target $ComputerName
            }
            catch {
                Write-Error `
