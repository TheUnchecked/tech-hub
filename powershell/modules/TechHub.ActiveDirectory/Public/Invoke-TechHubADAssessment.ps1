function Invoke-TechHubADAssessment {
    [CmdletBinding()]
    param (
        [Parameter()]
        [object]$Registry,
    ...
    if ($null -eq $Provider -and $ProviderRequired) {
        try {
            ...
            $Provider = New-TechHubADProvider -Server $Server -ErrorAction Stop
        }
        catch {
    ...
    foreach ($Definition in $Definitions) {
        ...
        if (-not $Definition.Enabled) {
            ...
            $CheckResult = New-TechHubADCheckExecutionError -Definition $Definition -Status 'NotAvailable' -ErrorType 'CheckDisabled' -ErrorMessage 'The check is disabled in the registry.' -StartedAt $CheckStartedAt -CompletedAt $CheckCompletedAt
        ...
        if ($Definition.IsReadOnly -ne $true) {
            $CheckCompletedAt = (Get-Date).ToUniversalTime()
            $CheckResult = New-TechHubADCheckExecutionError -Definition $Definition -Status 'Error' -ErrorType 'ReadOnlyViolation' -ErrorMessage 'The registered check does not declare IsReadOnly = $true.' -StartedAt $CheckStartedAt -CompletedAt $CheckCompletedAt
            [void]$Assessment.Metadata.CheckResults.Add($CheckResult)
        ...
        try {
            ...
            $CheckCompletedAt = (Get-Date).ToUniversalTime()
            $CheckResult = New-TechHubADCheckExecutionError -Definition $Definition -Status 'Error' -ErrorType 'FunctionNotFound' -ErrorMessage $_.Exception.Message -StartedAt $CheckStartedAt -CompletedAt $CheckCompletedAt
            [void]$Assessment.Metadata.CheckResults.Add($CheckResult)
            $Assessment.Summary.ChecksFailed++
            continue
        }
        if ($DefinitionRequiresProvider -and $null -eq $Provider -and
            ...
            $CheckCompletedAt = (Get-Date).ToUniversalTime()
            $CheckResult = New-TechHubADCheckExecutionError -Definition $Definition -Status 'Error' -ErrorType 'ProviderCreationError' -ErrorMessage $ProviderCreationError.Exception.Message -StartedAt $CheckStartedAt -CompletedAt $CheckCompletedAt
            [void]$Assessment.Metadata.CheckResults.Add($CheckResult)
            $Assessment.Summary.ChecksFailed++
            continue
        }
        ...
        try {
            ...
            $CheckResult = New-TechHubADCheckExecutionError -Definition $Definition -Status 'Error' -ErrorType $ErrorType -ErrorMessage $_.Exception.Message -StartedAt $CheckStartedAt -CompletedAt $CheckCompletedAt
            [void]$Assessment.Metadata.CheckResults.Add($CheckResult)
            $Assessment.Summary.ChecksFailed++
