function Get-TechHubADConstrainedDelegation {
    ...
    $Properties = @('DistinguishedName', 'Name', 'ObjectGUID', 'ObjectClass', 'ObjectCategory', 'SamAccountName', 'UserAccountControl', 'Enabled', 'ServicePrincipalName', 'msDS-AllowedToDelegateTo')
    ...
    if ([string]::IsNullOrWhiteSpace($DomainController)) { $DomainController = $Server }

    try {
        Write-Verbose -Message 'Requesting constrained delegation objects from TechHubADProvider.'
    ...
    if ($ProviderStatus -eq 'NotAvailable' -or $ProviderStatus -eq 'Error') {
        ...
        New-TechHubADFinding `
            ...
            -Evidence ([PSCustomObject][ordered]@{ ProviderStatus = $ProviderStatus; ErrorType = $ObjectsResult.ErrorType; ErrorMessage = $ObjectsResult.ErrorMessage; AllowedToDelegateTo = @(); AccountEnabled = $null; ServicePrincipalNames = @(); UserAccountControl = $null }) `
    ...
    Write-Verbose -Message ('Provider returned {0} object(s) with msDS-AllowedToDelegateTo.' -f $Objects.Count)
    foreach ($Object in $Objects) {
        ...
        if ($null -ne $Object.PSObject.Properties['msDS-AllowedToDelegateTo']) { $AllowedToDelegateTo = @($Object.PSObject.Properties['msDS-AllowedToDelegateTo'].Value | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }) }
        ...
        $CriticalMatches = @($AllowedToDelegateTo | Where-Object { $Destination = $_; @($CriticalServicePatterns | Where-Object { $Destination -like $_ }).Count -gt 0 })
        $DocumentedMatches = @($AllowedToDelegateTo | Where-Object { $Destination = $_; @($DocumentedServicePatterns | Where-Object { $Destination -like $_ }).Count -gt 0 })
        $ExcludedMatches = @($AllowedToDelegateTo | Where-Object { $Destination = $_; @($ExcludedServicePatterns | Where-Object { $Destination -like $_ }).Count -gt 0 })

        $Severity = 'Low'
        $Status = 'Finding'
        if ($Enabled -eq $false) { $Severity = 'Informational' }
        elseif ($CriticalMatches.Count -gt 0) { $Severity = 'High' }
        elseif ($AllowedToDelegateTo.Count -gt 1 -or ($DocumentedServicePatterns.Count -gt 0 -and $DocumentedMatches.Count -ne $AllowedToDelegateTo.Count)) { $Severity = 'Medium' }
        if ($ExcludedMatches.Count -eq $AllowedToDelegateTo.Count -and $ExcludedMatches.Count -gt 0) { $Severity = 'Informational'; $Status = 'NotApplicable' }

        $Confidence = 'High'
        if (-not $HasCompleteUac -or [string]::IsNullOrWhiteSpace($DistinguishedName) -or [string]::IsNullOrWhiteSpace($Name)) { $Confidence = 'Medium' }
        ...
        if ($Enabled -eq $false) { $Risk = 'The account is disabled, but the delegation configuration should be reviewed before the account is enabled.' }
        elseif ($CriticalMatches.Count -gt 0) { $Risk = 'The delegation targets a service matched by the supplied sensitive-service patterns and requires priority review.' }
