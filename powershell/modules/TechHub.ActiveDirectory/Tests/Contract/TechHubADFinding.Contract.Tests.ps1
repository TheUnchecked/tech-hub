...
. (Join-Path -Path $PSScriptRoot -ChildPath 'TechHubADContractTestHelpers.ps1')
...
Describe 'TechHub.ActiveDirectory finding contract v1' {
    ...
    BeforeEach {
        $Script:Domain = [PSCustomObject]@{ DNSRoot = 'example.test' }
        $Script:Forest = [PSCustomObject]@{ Name = 'example.test' }
    }
    ...
    Context 'Get-TechHubADUnconstrainedDelegation' {
        BeforeEach {
            ...
            Mock Get-ADObject -ModuleName TechHub.ActiveDirectory { @($Object) }
        }

        It 'returns output conforming to contract v1' {
            $Results = @(Get-TechHubADUnconstrainedDelegation)
            $Results.Count | Should -Be 1
            Assert-TechHubADFindingContract -Result $Results[0] -ExpectedCheckId 'AD-UNCONSTRAINED-DELEGATION' -ExpectedCheckName 'Unconstrained Delegation' -ExpectedCategory 'Delegation'
    ...
    Context 'Get-TechHubADConstrainedDelegation' {
        ...
        It 'returns output conforming to contract v1' {
            ...
            $Results.Count | Should -Be 1
            Assert-TechHubADFindingContract -Result $Results[0] -ExpectedCheckId 'AD-CONSTRAINED-DELEGATION' -ExpectedCheckName 'Constrained Delegation' -ExpectedCategory 'Delegation'
            Assert-TechHubADSafeOutput -Results $Results
        }
    ...
    Context 'Get-TechHubADRBCD' {
        BeforeEach {
            ...
            $Descriptor = New-TechHubADContractDescriptor -IdentityReference $TrusteeSid
            ...
            Mock Get-ADObject -ModuleName TechHub.ActiveDirectory {
                if ($PSBoundParameters.ContainsKey('Identity')) { $ResolvedTrustee }
                else { @($Target) }
            }
        }
        ...
        It 'returns output conforming to contract v1' {
            ...
            $Results.Count | Should -Be 1
            Assert-TechHubADFindingContract -Result $Results[0] -ExpectedCheckId 'AD-RBCD' -ExpectedCheckName 'Resource-Based Constrained Delegation' -ExpectedCategory 'Delegation'
            Assert-TechHubADSafeOutput -Results $Results
        }
    ...
    Context 'Get-TechHubADPrivilegedGroup' {
        BeforeEach {
            ...
            Mock Get-ADDomain -ModuleName TechHub.ActiveDirectory { $Script:Domain }
            Mock Get-ADForest -ModuleName TechHub.ActiveDirectory { $Script:Forest }
            Mock Get-ADGroup -ModuleName TechHub.ActiveDirectory { @($Group) }
            Mock Get-ADGroupMember -ModuleName TechHub.ActiveDirectory { @($Member) }
            Mock Get-ADObject -ModuleName TechHub.ActiveDirectory { $Detail }
        }

        It 'returns output conforming to contract v1' {
            $Results = @(Get-TechHubADPrivilegedGroup -IncludeDisabled)
            $Results.Count | Should -Be 1
            Assert-TechHubADFindingContract -Result $Results[0] -ExpectedCheckId 'AD-PRIVILEGED-GROUP' -ExpectedCheckName 'Privileged Groups' -ExpectedCategory 'PrivilegedAccess'
            Assert-TechHubADSafeOutput -Results $Results
        }
