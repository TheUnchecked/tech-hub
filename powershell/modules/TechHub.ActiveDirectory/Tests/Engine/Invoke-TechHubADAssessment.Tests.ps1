...
Describe 'Invoke-TechHubADAssessment provider lifecycle' {
    ...
    BeforeAll {
        ...
        if (-not (Test-Path -LiteralPath $ModuleManifest -PathType Leaf)) {
            throw "TechHub.ActiveDirectory manifest not found: $ModuleManifest"
        }
    ...
    BeforeEach {
        ...
        $Script:Registry = & (Get-Module TechHub.ActiveDirectory) {
            [TechHubADCheckRegistry]::new()
        }
    }

    It 'does not create a provider when no selected check requires one' {

        $Definition = & (Get-Module TechHub.ActiveDirectory) {
        ...
        Mock New-TechHubADProvider -ModuleName TechHub.ActiveDirectory {
            $Script:ProviderCreateCount++
            return $Script:Provider
        }

        $Result = Invoke-TechHubADAssessment `
    ...
    It 'reuses the exact same provider instance across provider-aware checks' {
        ...
        $DefinitionOne = & (Get-Module TechHub.ActiveDirectory) {
            [TechHubADCheckDefinition]::new(
                'AD-FIRST',
                'First',
                'Provider check.',
        ...
        $DefinitionTwo = & (Get-Module TechHub.ActiveDirectory) {
            [TechHubADCheckDefinition]::new(
                'AD-SECOND',
                'Second',
                'Provider check.',
    ...
    It 'preserves CheckId and category filtering' {
        ...
        $ResultById = Invoke-TechHubADAssessment `
            -Registry $Script:Registry `
            -CheckId 'AD-FIRST'
        ...
        $ResultByCategory = Invoke-TechHubADAssessment `
            -Registry $Script:Registry `
            -Category 'Health'
    ...
    It 'skips disabled checks and enforces read-only metadata' {
        ...
        $DisabledDefinition = & (Get-Module TechHub.ActiveDirectory) {
            [TechHubADCheckDefinition]::new(
                'AD-FIRST',
                'First',
                'Provider check.',
    ...
    It 'contains no direct AD modifications or dynamic execution' {
        ...
        $EnginePath = Join-Path `
            $ModuleRoot `
            'Public\Invoke-TechHubADAssessment.ps1'
