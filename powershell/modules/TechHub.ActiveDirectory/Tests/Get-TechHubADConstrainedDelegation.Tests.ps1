...
Describe 'Get-TechHubADConstrainedDelegation provider migration' {
    ...
    It 'skips objects with missing delegation targets' {
        ...
        @(Get-TechHubADConstrainedDelegation -Provider (New-TestConstrainedProvider -Objects @($NoTarget))).Count | Should -Be 0
    }

    It 'handles empty provider results' {
        @(Get-TechHubADConstrainedDelegation -Provider (New-TestConstrainedProvider)).Count | Should -Be 0
    }

    It 'preserves Available and Partial provider status' {
        $Available = @(Get-TechHubADConstrainedDelegation -Provider $Script:Provider)[0]
        $Available.Evidence.ProviderStatus | Should -Be 'Available'
        $Partial = @(Get-TechHubADConstrainedDelegation -Provider (New-TestConstrainedProvider -ObjectStatus 'Partial' -Objects @($Script:User)))[0]
    ...
    It 'returns structured results for NotAvailable and Error provider status' {
        $Unavailable = @(Get-TechHubADConstrainedDelegation -Provider (New-TestConstrainedProvider -ObjectStatus 'NotAvailable' -ErrorType 'ModuleUnavailable' -ErrorMessage 'Synthetic unavailable'))[0]
        $Unavailable.Status | Should -Be 'NotAvailable'
        $Unavailable.Evidence.ErrorType | Should -Be 'ModuleUnavailable'
        $ErrorResult = @(Get-TechHubADConstrainedDelegation -Provider (New-TestConstrainedProvider -ObjectStatus 'Error' -ErrorType 'LdapError' -ErrorMessage 'Synthetic LDAP failure'))[0]
    ...
    It 'handles incomplete objects and missing SPNs' {
        ...
        $Result = @(Get-TechHubADConstrainedDelegation -Provider (New-TestConstrainedProvider -Objects @($Incomplete)))[0]
    ...
    It 'handles enabled and disabled accounts' {
        ...
        $Result = @(Get-TechHubADConstrainedDelegation -Provider (New-TestConstrainedProvider -Objects @($Disabled)))[0]
    ...
    It 'supports configured severity patterns' {
        $Result = @(Get-TechHubADConstrainedDelegation -Provider $Script:Provider -CriticalServicePatterns 'HTTP/*')[0]
        $Result.Severity | Should -Be 'High'
        $Excluded = @(Get-TechHubADConstrainedDelegation -Provider $Script:Provider -ExcludedServicePatterns 'HTTP/*')[0]
    ...
    It 'passes Server and SearchBase to the provider' {
        ...
        Get-TechHubADConstrainedDelegation -Provider $Provider -Server 'dc01.example.test' -SearchBase 'OU=Service Accounts,DC=example,DC=test' | Out-Null
    ...
    It 'returns the complete Finding Contract v1 and remains read-only' {
        $Result = @(Get-TechHubADConstrainedDelegation -Provider $Script:Provider)[0]
